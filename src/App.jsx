...
async function genQuestion(difficulty,domain,history,tickerIdx){
  ...
  const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":import.meta.env.VITE_ANTHROPIC_KEY,"anthropic-version":"2023-06-01","anthropic-dangerous-direct-browser-access":"true"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1000,system:QUIZ_PROMPT,messages:[{role:"user",content:`Difficulty ${difficulty}, domain: ${domain}, ticker: ${ticker}.${histStr}`}]})});
    ...
    <div style={{paddingBottom:80}}>
      <div style={{padding:"20px 18px 14px",background:`linear-gradient(180deg,${T.goldDim} 0%,transparent 100%)`,borderBottom:`1px solid ${T.border}`}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          <div><div style={{fontSize:22,fontWeight:"bold",letterSpacing:-0.5}}>Good morning.</div><div style={{fontSize:11,color:T.dim,marginTop:1}}>May 13, 2026</div></div>
...
function LearnHub({onSelectModule}){
  ...
  return(
    <div style={{padding:"18px 20px 80px"}}>
      <div style={{fontSize:12,color:T.dim,lineHeight:1.65,marginBottom:20}}>Eight modules. Start anywhere. The framework gets better the more you understand it.</div>
      {modules.map(m=>(
