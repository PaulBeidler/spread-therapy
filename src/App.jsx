...
const USER = {
  ...
  monthlyGoal: 2500,
  annualGoal: 30000,
  goalPurpose: "Private school tuition · 3 children",
  profile: "Moderate",
  vix: 18.16,
  rut: 2881,
  date: "May 12, 2026",
...
const CAMPAIGNS = [
  {
    id: "ST-002", ticker: "MU", name: "Covered Call Campaign",
    type: "covered_call", status: "active", tier: 1,
    truePnL: 55218, currentCall: "$1,000 Call · Oct 16",
    dteApprox: 157, otmPct: 26.9, delta: 0.08, health: "green",
    thesis: "Long-term semiconductor conviction. Selling calls to generate income while shares appreciate.",
  },
  {
    id: "ST-001", ticker: "IONQ", name: "Naked Put — Rogue",
    type: "naked_put", status: "active", tier: null,
    netPremium: 509, strikes: "$55 Put · Jun 5",
    dteApprox: 24, otmPct: 5.6, delta: 0.3937, health: "red",
    alert: "Delta 0.39 — hard stop violation. Close before camping trip May 22.",
    thesis: "High IV entry. Rogue — outside framework.",
  },
  {
    id: "ST-004", ticker: "RUTW", name: "Bear Call Spread",
    type: "bear_call", status: "active", tier: 3,
    netPremium: 290, strikes: "3080/3100 · Jun 18",
    dteApprox: 37, otmPct: 6.9, delta: -0.025, health: "yellow",
    alert: "3 consecutive up days. Watch if RUT > 2,980.",
    thesis: "Tactical hedge on extended small-cap market.",
  },
  {
    id: "ST-005", ticker: "RUT", name: "LEAP Crash Shield",
    type: "leap", status: "active", tier: 4,
    netPremium: -2836, strikes: "2350/2150 · Jun 2027",
    dteApprox: 401, otmPct: 18.4, delta: -0.165, health: "green",
    thesis: "Offensive capital for crash re-entry. Not insurance — ammunition.",
  },
  {
    id: "ST-006", ticker: "LMT", name: "Bull Put Spread",
    type: "bull_put", status: "active", tier: 2,
    netPremium: 400, strikes: "$480/$470 · Jun 18",
    dteApprox: 37, otmPct: 6.4, delta: 0.12, health: "green",
    thesis: "Defense sector bull put. LMT comfortable buffer.",
  },
  {
    id: "ST-007", ticker: "INTC", name: "Bull Put Spread",
    type: "bull_put", status: "active", tier: 2,
    netPremium: 376, strikes: "$90/$80 · Jun 18",
    dteApprox: 37, otmPct: 20.4, delta: 0.07, health: "green",
    thesis: "Parabolic stock. 20% OTM — very safe. Harvesting premium on the excitement.",
  },
];

const HISTORY = [
  {
    id: "C-001", ticker: "RUTW", name: "Bull Put 2770/2750",
    closedDate: "May 7, 2026", pnl: 54, emotion: "😰",
    lesson: "Entered at 1.9% OTM on gap-up morning — violated 3 rules simultaneously. Survived. Closed with dignity.",
    violations: ["Gap-up entry", "Delta 0.32", "<3% OTM"],
  },
  {
    id: "C-002", ticker: "CAT", name: "Covered Call + Naked Puts",
    closedDate: "Apr 10, 2026", pnl: 577, opportunityCost: 16523, emotion: "😔",
    lesson: "Made $577. Lost $16,523 I never knew I was losing. CAT went from $740 to $911. The invisible loss.",
    violations: ["Naked puts", "Single stock", "Panic management"],
  },
];
...
const fmtK = (n) => {
  ...
  const s = abs >= 10000 ? `${(abs/1000).toFixed(0)}K` : abs >= 1000 ? `${(abs/1000).toFixed(1)}K` : `${abs}`;
  return n >= 0 ? `+$${s}` : `-$${s}`;
};
const fmtFull = (n) => {
  if (n == null) return "—";
...
function updateAbility(ability, difficulty, correct) {
  ...
  return ability + 0.5 * ((correct ? 1 : 0) - p);
}
function abilityToScore(ability) {
  return Math.max(0, Math.min(100, Math.round(((ability + 3) / 6) * 100)));
}
function nextDiff(ability) {
  return Math.max(1, Math.min(10, Math.round(ability + 5)));
}
function getLevel(score) {
  if (score < 30) return { label: "Beginner", color: T.blue, icon: "◎" };
  if (score < 55) return { label: "Developing", color: T.green, icon: "◑" };
  if (score < 75) return { label: "Intermediate", color: T.gold, icon: "◕" };
  if (score < 90) return { label: "Advanced", color: T.orange, icon: "●" };
...
async function genQuestion(difficulty, domain, history, tickerIdx) {
  ...
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      max_tokens: 1000,
      system: QUIZ_PROMPT,
      messages: [{ role: "user", content: `Difficulty ${difficulty}, domain: ${domain}, ticker: ${ticker}.${histStr}` }]
    })
  });
...
const Tag = ({ label, color }) => (
  <span style={{ fontSize: 9, letterSpacing: 1, color: color || T.gold, background: `${color || T.gold}18`, padding: "2px 8px", borderRadius: 10, border: `1px solid ${color || T.gold}30` }}>{label}</span>
);

const SectionHead = ({ title, color, count }) => (
  <div style={{ fontSize: 10, color: color || T.dim, letterSpacing: 2, textTransform: "uppercase", marginBottom: 10 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginRight: 18 }}>
        ...
        {pnl != null && (
          <div style={{ textAlign: "right" }}>
            <div style={{ fontSize: 15, fontFamily: "monospace", color: pnl >= 0 ? T.green : T.red }}>{fmtK(pnl)}</div>
            <div style={{ fontSize: 9, color: T.dim }}>{c.truePnL != null ? "true P&L" : "net premium"}</div>
          </div>
        )}
      </div>
  ...
  return (
    <div style={{ minHeight: "100vh", background: T.bg, color: T.text, fontFamily: "Georgia, serif", maxWidth: 480, margin: "0 auto" }}>
      <div style={{ padding: "18px 18px 14px", borderBottom: `1px solid ${T.border}`, display: "flex", alignItems: "center", gap: 12, position: "sticky", top: 0, background: T.bg, zIndex: 10 }}>
        ...
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <span style={{ fontSize: 18, fontWeight: "bold" }}>{c.ticker}</span>
            <Tag label={TYPE_LABEL[c.type]} color={tc} />
          </div>
          <div style={{ fontSize: 11, color: T.dim }}>{c.id} · {c.status}</div>
        </div>
      </div>
      <div style={{ padding: "18px 18px 60px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 16 }}>
          {[["DTE", c.dteApprox], ["OTM", `${c.otmPct}%`], ["Delta", c.delta != null ? Math.abs(c.delta).toFixed(3) : "—"], ["Status", c.status]].map(([l, v]) => (
            <div key={l} style={{ background: T.surface, borderRadius: 8, padding: "10px 12px" }}>
              <div style={{ fontSize: 9, color: T.dim, letterSpacing: 2, marginBottom: 3 }}>{l}</div>
              <div style={{ fontSize: 16, color: T.text, fontFamily: "monospace" }}>{v}</div>
            </div>
          ))}
        </div>
        ...
        {c.alert && (
          <div style={{ background: `${hc}10`, border: `1px solid ${hc}30`, borderRadius: 10, padding: 12, marginBottom: 16 }}>
            <div style={{ fontSize: 10, color: hc, letterSpacing: 2, marginBottom: 4 }}>ALERT</div>
            <div style={{ fontSize: 12, color: hc, lineHeight: 1.5 }}>{c.alert}</div>
          </div>
        )}
        <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 10, padding: 14 }}>
          <div style={{ fontSize: 10, color: T.dim, letterSpacing: 2, marginBottom: 10 }}>MANAGEMENT RULES</div>
          {[["50% profit target", "Close immediately — no exceptions"], ["21 DTE rule", c.dteApprox <= 21 ? "⚠ Evaluate now" : `${c.dteApprox - 21} days until review`], ["Camping trip", c.dteApprox <= 10 ? "🏕 Resolve before May 22" : "Safe — expires after return"]].map(([rule, val]) => (
            <div key={rule} style={{ display: "flex", justifyContent: "space-between", padding: "7px 0", borderBottom: `1px solid ${T.border}` }}>
              <span style={{ fontSize: 11, color: T.dim }}>{rule}</span>
              <span style={{ fontSize: 11, color: val.includes("⚠") || val.includes("🏕") ? T.amber : T.mid }}>{val}</span>
            </div>
          ))}
        </div>
      </div>
...
// ── DASHBOARD ─────────────────────────────────────────────────────────────────
function Dashboard({ onSelectCampaign, onNav }) {
  ...
  const monthPct = Math.min((monthlyIncome / USER.monthlyGoal) * 100, 100);
    ...
    <div style={{ paddingBottom: 80 }}>
      ...
      <div style={{ padding: "20px 18px 14px", background: `linear-gradient(180deg, ${T.goldDim} 0%, transparent 100%)`, borderBottom: `1px solid ${T.border}` }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div>
            <div style={{ fontSize: 10, color: T.gold, letterSpacing: 4, textTransform: "uppercase", marginBottom: 3 }}>Spread Therapy</div>
            <div style={{ fontSize: 22, fontWeight: "bold", letterSpacing: -0.5 }}>Good morning.</div>
            <div style={{ fontSize: 11, color: T.dim, marginTop: 1 }}>{USER.date}</div>
          </div>
          <div style={{ textAlign: "right" }}>
            <div style={{ fontSize: 24, fontWeight: "bold" }}>${(USER.accountValue/1000).toFixed(1)}K</div>
            <div style={{ fontSize: 10, color: T.red }}>-$3,418 today</div>
          </div>
        </div>
        <div style={{ marginTop: 14 }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5 }}>
            <span style={{ fontSize: 10, color: T.dim, letterSpacing: 1 }}>MONTHLY · {USER.goalPurpose}</span>
          ...
          <div style={{ background: "rgba(255,255,255,0.06)", borderRadius: 3, height: 3 }}>
            <div style={{ background: `linear-gradient(90deg, ${T.gold}, #f0d080)`, borderRadius: 3, height: 3, width: `${monthPct}%` }} />
          </div>
          <div style={{ fontSize: 10, color: T.dim, marginTop: 3 }}>{Math.round(monthPct)}% of monthly · $30K annual goal</div>
        </div>
        <div style={{ display: "flex", gap: 6, marginTop: 12 }}>
          {[{ l: "VIX", v: USER.vix, c: T.green, n: "✓ range" }, { l: "RUT", v: USER.rut.toLocaleString(), c: T.blue, n: "+3 days up" }, { l: "Bull Put", v: "Wait", c: T.dim, n: "need ↓ day" }, { l: "Bear Call", v: "Ready", c: T.purple, n: "3 up days" }].map(m => (
      ...
      <div style={{ padding: "14px 16px" }}>
        {tab === "today" && (
          <>
            ...
            {alerts.length > 0 && <>
              <SectionHead title="Needs Attention" color={T.red} count={alerts.length} />
              {alerts.map(c => <CampaignCard key={c.id} c={c} onSelect={onSelectCampaign} />)}
        ...
        {tab === "campaigns" && (
          <>
            {[[1,"Tier 1 — Covered Calls",T.green],[2,"Tier 2 — Bull Puts",T.blue],[3,"Tier 3 — Bear Calls",T.purple],[4,"Tier 4 — Crash Shield",T.gold],[null,"🔴 Rogue",T.red]].map(([tier,label,color]) => {
              ...
              return <div key={String(tier)} style={{ marginBottom: 4 }}>
                <SectionHead title={label} color={color} />
                {group.map(c => <CampaignCard key={c.id} c={c} onSelect={onSelectCampaign} />)}
        ...
        {tab === "history" && (
          <>
            <div style={{ fontSize: 12, color: T.dim, marginBottom: 16, lineHeight: 1.6 }}>Every closed campaign. The wins, the lessons, the numbers your brokerage never showed you.</div>
            {HISTORY.map(c => (
              <div key={c.id} style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 12, padding: 15, marginBottom: 12 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                  <div>
                    <div style={{ fontSize: 15, fontWeight: "bold" }}>{c.ticker}</div>
                    <div style={{ fontSize: 11, color: T.dim }}>{c.name} · {c.closedDate}</div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <div style={{ fontSize: 18, fontFamily: "monospace", color: c.pnl >= 0 ? T.green : T.red }}>{fmtFull(c.pnl)}</div>
                    {c.emotion && <div style={{ fontSize: 16, marginTop: 2 }}>{c.emotion}</div>}
                  </div>
                </div>
                {c.opportunityCost && (
                  <div style={{ background: "rgba(239,68,68,0.06)", border: "1px solid rgba(239,68,68,0.15)", borderRadius: 8, padding: 10, marginBottom: 10 }}>
                    <div style={{ fontSize: 9, color: T.red, letterSpacing: 1, marginBottom: 6 }}>INVISIBLE LOSS — OPPORTUNITY COST</div>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
                      <span style={{ fontSize: 11, color: T.mid }}>Campaign P&L</span>
                      <span style={{ fontSize: 11, color: T.green, fontFamily: "monospace" }}>{fmtFull(c.pnl)}</span>
                    </div>
                    <div style={{ display: "flex", justifyContent: "space-between" }}>
                      <span style={{ fontSize: 11, color: T.mid }}>What you left behind</span>
                      <span style={{ fontSize: 11, color: T.red, fontFamily: "monospace" }}>{fmtFull(c.opportunityCost)}</span>
                    </div>
                  </div>
                )}
...
function Quiz({ onBack }) {
  ...
  const prefetchNext = useCallback((num, ab, hist, tIdx) => {
    ...
    setNext(null);
    const p = genQuestion(nextDiff(ab), domain, hist, tIdx)
      .then(q => { if (prefetchRef.current === p) setNext(q); })
      .catch(() => {});
  ...
  const loadQ = useCallback(async (num, ab, hist) => {
    ...
    try {
      const domain = DOMAINS[(num - 1) % DOMAINS.length];
      const tIdx = tickerIdx.current;
      tickerIdx.current = tIdx + 1;
      const q = await genQuestion(nextDiff(ab), domain, hist, tIdx);
      setCurrent(q);
      prefetchNext(num + 1, ab, hist, tickerIdx.current);
  ...
  if (phase === "intro") return (
    <div style={{ padding: "24px 20px 80px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 24 }}>
        <button onClick={onBack} style={{ background: "none", border: "none", color: T.gold, fontSize: 20, cursor: "pointer" }}>←</button>
        <div style={{ fontSize: 18, fontWeight: "bold" }}>Knowledge Assessment</div>
      </div>
      ...
      {[["◎",T.blue,"Beginner","Sandbox mode. Education first."],["◑",T.green,"Developing","Paper trading unlocked."],["◕",T.gold,"Intermediate","Full platform access."],["●",T.orange,"Advanced","All strategies."],["★",T.purple,"Expert","Framework contributor."]].map(([icon,color,label,desc]) => (
      ...
      <button onClick={handleStart} style={{ width: "100%", marginTop: 24, padding: "15px", background: T.goldDim, border: `1px solid ${T.goldBorder}`, borderRadius: 10, color: T.gold, fontSize: 14, cursor: "pointer", fontFamily: "Georgia, serif" }}>
        Begin Assessment →
      </button>
    </div>
  );
  ...
  if (phase === "result") return (
    <div style={{ padding: "0 0 60px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "18px 20px 14px", borderBottom: `1px solid ${T.border}` }}>
        <button onClick={onBack} style={{ background: "none", border: "none", color: T.gold, fontSize: 20, cursor: "pointer" }}>←</button>
        <div style={{ fontSize: 16, fontWeight: "bold" }}>Your Results</div>
      </div>
        ...
        {Object.entries(domainScores).filter(([_,d]) => d.attempts > 0).map(([domain, data]) => {
          ...
          return <div key={domain} style={{ marginBottom: 10 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
              <span style={{ fontSize: 11, color: T.mid }}>{domainDisplay[domain]}</span>
              <span style={{ fontSize: 11, color: bc, fontFamily: "monospace" }}>{data.correct}/{data.attempts}</span>
            </div>
            <div style={{ background: "rgba(255,255,255,0.05)", borderRadius: 2, height: 2 }}>
              <div style={{ background: bc, height: 2, width: `${pct}%`, borderRadius: 2 }} />
            </div>
          </div>;
        })}
        <div style={{ marginTop: 20 }}>
          ...
          {history.map((h, i) => (
            <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "8px 10px", borderRadius: 7, marginBottom: 4, background: T.surface, borderLeft: `2px solid ${h.correct ? T.green : T.red}` }}>
              ...
              <div>
                <div style={{ fontSize: 11, color: T.mid }}>{h.concept}</div>
                <div style={{ fontSize: 10, color: T.dim }}>{h.ticker && <span style={{ marginRight: 6 }}>{h.ticker}</span>}d{h.difficulty} · {domainDisplay[h.domain]}</div>
              </div>
            </div>
          ))}
        </div>
    ...
    <div style={{ padding: "18px 20px 80px" }}>
      ...
      <div style={{ marginBottom: 20 }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
          <span style={{ fontSize: 11, color: T.dim }}>Q {qNum} / {TOTAL_Q}</span>
          <span style={{ fontSize: 11, color: level.color }}>{score}% · {level.label}</span>
        </div>
        <div style={{ background: "rgba(255,255,255,0.06)", borderRadius: 2, height: 2 }}>
          <div style={{ background: level.color, height: 2, width: `${(qNum / TOTAL_Q) * 100}%`, transition: "width 0.5s ease", borderRadius: 2 }} />
        </div>
      </div>

      {loading && (
        <div style={{ textAlign: "center", padding: "60px 20px" }}>
          <div style={{ fontSize: 28, marginBottom: 14, animation: "spin 2s linear infinite" }}>◎</div>
          <div style={{ fontSize: 12, color: T.dim }}>Calibrating to your level...</div>
          <style>{`@keyframes spin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}`}</style>
        </div>
      )}
      {error && <div style={{ background: "rgba(239,68,68,0.07)", border: `1px solid rgba(239,68,68,0.18)`, borderRadius: 8, padding: 14, textAlign: "center" }}>
        <div style={{ fontSize: 12, color: T.red, marginBottom: 10 }}>{error}</div>
        <button onClick={() => loadQ(qNum, ability, history)} style={{ padding: "9px 18px", background: "rgba(239,68,68,0.08)", border: `1px solid rgba(239,68,68,0.25)`, borderRadius: 6, color: T.red, fontSize: 11, cursor: "pointer", fontFamily: "Georgia, serif" }}>Try again</button>
      </div>}
      ...
      {!loading && !error && current && (
        <>
          ...
          {revealed && (
            <div style={{ background: selected === current.correct ? "rgba(34,197,94,0.05)" : "rgba(239,68,68,0.05)", border: `1px solid ${selected === current.correct ? "rgba(34,197,94,0.18)" : "rgba(239,68,68,0.18)"}`, borderRadius: 10, padding: 14, marginBottom: 12 }}>
      ...
      {!loading && !error && current && (
        <div style={{ position: "fixed", bottom: 60, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 480, background: "rgba(12,12,16,0.97)", backdropFilter: "blur(12px)", borderTop: `1px solid ${T.border}`, padding: "12px 20px" }}>
          {!revealed
...
function LearnHub({ onBack }) {
  ...
  return (
    <div style={{ padding: "18px 20px 80px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 24 }}>
        <button onClick={onBack} style={{ background: "none", border: "none", color: T.gold, fontSize: 20, cursor: "pointer" }}>←</button>
        <div style={{ fontSize: 18, fontWeight: "bold" }}>Learn Hub</div>
      </div>
      <div style={{ fontSize: 12, color: T.dim, lineHeight: 1.65, marginBottom: 20 }}>
        Seven modules. Start anywhere. The framework gets better the more you understand it.
      </div>
      {modules.map(m => (
        <div key={m.id} style={{ background: T.surface, border: `1px solid ${T.border}`, borderLeft: `3px solid ${m.color}`, borderRadius: 10, padding: "14px 16px", marginBottom: 8, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <div style={{ textAlign: "right", flexShrink: 0 }}>
            ...
            <div style={{ fontSize: 10, color: T.dim, marginTop: 4 }}>Coming soon</div>
          </div>
        </div>
      ))}
      <div style={{ marginTop: 20, background: T.goldDim, border: `1px solid ${T.goldBorder}`, borderRadius: 12, padding: 16, textAlign: "center" }}>
        <div style={{ fontSize: 13, color: T.gold, marginBottom: 6 }}>Start with the assessment</div>
        <div style={{ fontSize: 12, color: T.dim, lineHeight: 1.5 }}>Take the quiz to find your knowledge gaps before choosing where to start.</div>
      </div>
    </div>
  );
}
...
export default function App() {
  const [screen, setScreen] = useState("home"); // home | detail | quiz | learn
  const [selectedCampaign, setSelectedCampaign] = useState(null);
  const [navTab, setNavTab] = useState("home"); // home | quiz | learn
  ...
  if (screen === "detail" && selectedCampaign) return (
    <div style={{ minHeight: "100vh", background: T.bg, color: T.text, fontFamily: "Georgia, serif", maxWidth: 480, margin: "0 auto" }}>
      <CampaignDetail c={selectedCampaign} onBack={handleBack} />
    </div>
  );
  ...
  return (
    <div style={{ minHeight: "100vh", background: T.bg, color: T.text, fontFamily: "Georgia, serif", maxWidth: 480, margin: "0 auto" }}>
      ...
      {/* Header */}
      <div style={{ padding: "16px 20px 12px", borderBottom: `1px solid ${T.border}`, position: "sticky", top: 0, zIndex: 20, background: "rgba(12,12,16,0.96)", backdropFilter: "blur(12px)" }}>
        <div style={{ fontSize: 9, color: T.gold, letterSpacing: 4, textTransform: "uppercase", marginBottom: 2 }}>Spread Therapy</div>
        <div style={{ fontSize: 15, fontWeight: "bold" }}>
          {navTab === "home" ? "Dashboard" : navTab === "quiz" ? "Knowledge Assessment" : "Learn Hub"}
        </div>
      </div>

      {/* Content */}
      <div>
        {navTab === "home" && <Dashboard onSelectCampaign={handleSelectCampaign} onNav={setNavTab} />}
        {navTab === "quiz" && <Quiz onBack={() => setNavTab("home")} />}
        {navTab === "learn" && <LearnHub onBack={() => setNavTab("home")} />}
      </div>
